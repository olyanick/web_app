...
module.exports = {
  getFiles: function (req, res) {
    Files.find({user: req.user.id, parent: 0}).populate("children").populate("groups").exec(function (err, files) {
      if (files.length == 0) {
  ...
  saveFile: function (req, res) {
    if (req.body.action == "create") {
      ...
      req.file('upload').upload({
        ...
        if (!req.body.target) {
          Files.findOne({name: "uploads", user: req.user.id}, function (err, uploads) {
            if (err) throw err;
            if (!uploads) {
              Files.findOne({user: req.user.id, parent: 0}, function (err, root) {
                if (err) throw  err;
                Files.create({name: "uploads", user: req.user.id, parent: root.id}, function (err, uploads) {
                  if (err) throw err;
                  var newFile = {
  ...
  getFile: function (req, res) {
    ...
    Files.findOne({id: id}, function (err, file) {
      ...
      if(file.user == req.user.id){
        return _this.fileSend(req,res,file);
      }
    
      if(file.access == "private"){
        return res.end("Access denied");
      } else if(file.access == "public"){
        return _this.fileSend(req,res,file);
      } else {
        FileAccess.find({file:id}).exec(function(err,accesses){
         var groups = _.pluck(accesses,"group");
          GroupMember.find({group:groups,user:req.user.id},function(err,member){
            if(err) throw err;
            if(member.length > 0){
